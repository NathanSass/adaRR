...
document.getElementById('generate').addEventListener('click', () => {
  ...
  const room = {
    x: width,
    y: depth,
    door: { pos1: { x: doorX, y: 0 }, pos2: { x: doorX + doorW, y: 0 } }
  };
...
function drawPreview() {
  ...
  // Walls (thick, with door gap)
  ctx.strokeStyle = '#1e293b'; ctx.lineWidth = 3; ctx.lineCap = 'square';
  // Top left of door
  ctx.beginPath(); ctx.moveTo(toX(0), toY(0)); ctx.lineTo(toX(doorState.x), toY(0)); ctx.stroke();
  // Top right of door
  ctx.beginPath(); ctx.moveTo(toX(doorState.x + doorState.width), toY(0)); ctx.lineTo(toX(roomW), toY(0)); ctx.stroke();
  // Right
  ...
  // Door panel (thicker than wall, from hinge down)
  ctx.strokeStyle = '#1e293b'; ctx.lineWidth = 4; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(toX(doorState.x), toY(0));
  ctx.lineTo(toX(doorState.x), toY(doorState.width));
  ctx.stroke();

  // Door swing arc
  ctx.strokeStyle = '#1e293b'; ctx.lineWidth = 1; ctx.lineCap = 'butt';
  ctx.beginPath();
  ctx.arc(toX(doorState.x), toY(0), toW(doorState.width), 0, Math.PI/2);
  ctx.stroke();

  // Drag handle (blue dot at center of door opening)
  ctx.fillStyle = isDragging ? '#1d4ed8' : '#3b82f6';
  ctx.beginPath();
  ctx.arc(toX(doorState.x + doorState.width/2), toY(0), 6, 0, Math.PI*2);
  ctx.fill();
...
function getDoorHitbox() {
  const x = previewOffsetX + doorState.x * previewScale;
  const w = doorState.width * previewScale;
  return { x: x-8, y: previewOffsetY-12, w: w+16, h: 24 };
}
...
function onDoorMouseMove(e) {
  ...
  const mx = e.clientX - rect.left, my = e.clientY - rect.top;
  if (isDragging) {
    const roomX = (mx - previewOffsetX) / previewScale - doorState.width/2;
    doorState.x = Math.max(0, Math.min(roomX, currentRoomW - doorState.width));
    document.getElementById('door-x').value = doorState.x.toFixed(1);
    drawPreview();
  } else {
    const h = getDoorHitbox();
